<template>
  <view class="goods_info">
    ...
    <nut-swiper
        ...
        height="375"
    >
      <nut-swiper-item v-for="(banner, index) in state.banners" :key="index">
        <img class="item_img" :src="state.imgUrlHeader + banner" alt=""/>
      </nut-swiper-item>
      <template v-slot:page>
        <div class="page">{{ state.current }}/{{ state.banners.length }}</div>
      </template>
    </nut-swiper>
    <view class="price_wrap">
      ...
      <view class="opt_wrap">
        <view class="btn">
          <icon-font name="s-follow"></icon-font>
          <view>收藏</view>
        </view>
        <view class="btn">
          <icon-font name="share"></icon-font>
          <view>分享</view>
        </view>
      </view>
    </view>
